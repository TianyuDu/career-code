...
def collate(samples, pad_idx, eos_idx, fixed_pad_length=None, pad_to_bsz=None):
    ...
    def merge(key, is_list=False):
        if is_list:
            raise ValueError("Cannot be list.")
        else:
            if len(samples) > 0 and samples[0][key] is None:
              return None
...
class WorkerDataset(FairseqDataset):
    """
    A wrapper around torch.utils.data.Dataset for worker data.
    ...
    def __init__(
        ...
        self.dataset = job_dataset
        self.year_dataset = year_dataset
        self.education_dataset = education_dataset
    ...
    def __getitem__(self, index):
        if self.targets is not None:
            ...
            # Getting the [1] index of the time-varying covariates gets the
            # right-shifted versions.
            year = self.year_dataset[index][1] if self.year_dataset is not None else None  
            education = self.education_dataset[index][1] if self.education_dataset is not None else None
